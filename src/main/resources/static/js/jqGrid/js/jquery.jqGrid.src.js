	...
	return this.each( function() {
		...
		addXmlData = function (xml,t, rcnt, more, adjust) {
			...
			while (j<gl) {
				...
				rowData.push("");
				if( gi ) {
					rowData.push( addMulti(rid,0,j, false) );
				}
				if( ni ) {
					rowData.push( addRowNum(gi,j,ts.p.page,ts.p.rowNum) );
				}
				if( si ) {
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			while (i<len) {
				...
				rowData.push("");
				if( gi ){
					rowData.push( addMulti(idr,0,i,selr) );
				}
				if( ni ) {
					rowData.push( addRowNum(gi,i,ts.p.page,ts.p.rowNum) );
				}
				if( si ) {
		...
		setPager = function (pgid, tp){
			...
			if(ts.p.pgbuttons===true ) {
				...
				pgl += "<td id='"+po[0]+"' class='ui-pg-button ui-corner-all'><span class='pageButton'>首页</span></td>";
				pgl += "<td id='"+po[1]+"' class='ui-pg-button ui-corner-all'></span><span class='pageButton'>上一页</span></td>";
				pgl += pginp !== "" ? sep+pginp+sep:"";
				pgl += "<td id='"+po[2]+"' class='ui-pg-button ui-corner-all'><span class='pageButton'>下一页</span></td>";
				pgl += "<td id='"+po[3]+"' class='ui-pg-button ui-corner-all'><span class='pageButton'>尾页</span></td>";
			...
			$('input.ui-pg-input',"#"+pgcnt).keypress( function(e) {
				...
				if(key == 13) {
					...
					if(!r.test($(this).val())){
						...
						window.parent.layer.alert("非法字符", 1);
						
					}else{
						
						if($(this).val()>ts.p.lastpage){
							
							window.parent.layer.alert("页码1到"+ts.p.lastpage+"之间", 1);
